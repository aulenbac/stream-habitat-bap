...
#file<- paste(wd,"Data/All_Data.csv", sep='')
data <- read.csv(file)
name <- names(data)
              ...
              # a tab for a map 
               tabPanel("Map", leafletOutput(outputId="map", width = "100%", height= 900), 
                      absolutePanel(
                        ...
                        selectInput(inputId='metric', label="Select Metric", choices=metrics_list$ShortName[c(12,13,14,16,17,18)], selected='PctPool'),
   ...
   #create simple scatter plot 
       output$plot<- renderPlot({
              nvcs_ec = data %>% 
                  ...
                  ggplot(nvcs_ec, aes(x=nvcs_ec[,1], y=nvcs_ec[,2], color=Program))+geom_point()+ labs(x=input$metric, y = input$metric_y)
                 #plot(nvcs_ec)
         })
    ...
    #create a histogram 
        output$hist <- renderPlot({
                ...
                #  ggplot(nvcs_h, aes(input$metric))+ geom_point()+facet_wrap(~Program)
                  qplot(nvcs_h[,1], geom='histogram')+ labs(x=input$metric)
                  })
       
#Second Tab to pull method data from MR.org using the APIs 
        file_metadata<- paste(wd,"Code/data_organize/metric_documentation.R", sep='')
        source(file_metadata)
    
  #Need help formating the html table 
    output$Methods <- renderDataTable (datatable({ 
